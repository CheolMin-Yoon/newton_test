"""CartPole Balance environment configuration.

This is a faithful copy of the built-in mjlab `Mjlab-Cartpole-Balance` task,
relocated into the tutorial package. The reward/observation/dynamics are
identical to mjlab's dm_control-style cartpole; only the scene entity key is
"robot" (tutorial convention) instead of "cartpole".
"""
...
from ...robots.cartpole.cartpole_constants import get_cartpole_robot_cfg
...
_ENTITY = "robot"
_CART_CFG = SceneEntityCfg(_ENTITY, joint_names=("slider",))
_HINGE_CFG = SceneEntityCfg(_ENTITY, joint_names=("hinge_1",))
...
# ==============================================================================
# Rewards (dm_control smooth cartpole reward)
# ==============================================================================
...
def _make_env_cfg() -> ManagerBasedRlEnvCfg:
  cart_cfg = SceneEntityCfg(_ENTITY, joint_names=("slider",))
  hinge_cfg = SceneEntityCfg(_ENTITY, joint_names=("hinge_1",))
  ...
  events = {
    "reset_slider": EventTermCfg(
      ...
      params={
        ...
        "asset_cfg": SceneEntityCfg(_ENTITY, joint_names=("slider",)),
      },
    ),
    "reset_hinge": EventTermCfg(
      ...
      params={
        ...
        "velocity_range": (-0.01, 0.01),
        "asset_cfg": SceneEntityCfg(_ENTITY, joint_names=("hinge_1",)),
      },
    ),
  }
  ...
  return ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      ...
      entities={_ENTITY: get_cartpole_robot_cfg()},
      num_envs=1,
      env_spacing=4.0,
    ),
...
def cartpole_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create CartPole Balance environment configuration.

  Args:
    play: If True, disables observation corruption and extends episode length.
  """
  cfg = _make_env_cfg()
  if play:
