...
"""
Featherstone diff-sim + 지면 접촉 (Go2 stand pose, 발 ground 접촉 상태).

목적: 공중에서 작동한 Featherstone diff sim이, ground contact가 끼면
       hang / NaN / grad=0 중 어디서 끊기는지 정확히 확인.
...
print(f"[contact-diff] URDF: {URDF_PATH}")
...
class Example:
    def __init__(self, viewer, args):
        ...
        # 발 접촉 soft 하게: spring + damper + friction
        ...
        self.target_dof_idx = wp.array([i + 1 for i in act_dof_idx], dtype=int)
        target_vals = np.array(STAND_PIN_ORDER, dtype=np.float32) + 0.3
        self.target_q = wp.array(target_vals, dtype=wp.float32)

        self.train_iter = 0
        self.train_rate = 50.0
        self.loss_history = []
    ...
    def forward_backward(self):
        ...
        wp.synchronize()
        t_fwd_end = time.time()
        print(f"  forward {(t_fwd_end - t_fwd_start)*1000:.1f}ms", flush=True)

        t_bwd_start = time.time()
        self.tape.backward(self.loss)
        wp.synchronize()
        t_bwd_end = time.time()
        print(f"  backward {(t_bwd_end - t_bwd_start)*1000:.1f}ms", flush=True)
    ...
    def step(self):
        ...
        print(f"[iter {self.train_iter:3d}] loss={loss_val:+.4f}  |grad|max={grad_max:.4e}  nan={grad_has_nan}", flush=True)
        self.loss_history.append(loss_val)

        valid = (self.tau.grad is not None) and (not grad_has_nan) and (grad_max > 1e-12)
        if valid:
            g_clipped = np.clip(g_np, -1e2, 1e2)
